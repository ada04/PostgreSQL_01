...
```pgsql
select * from tab_arr where exists (select 1 from getListTel(tel) where tel like '%+7910%');
```

#### Function SQL Table

```pgsql
select bp.boarding_no, bp.seat_no, s.fare_conditions, t.passenger_name,
             t.contact_data -> 'phone' as phone, f.flight_id  
...
```pgsql
select tf.flight_id, bp.boarding_no, bp.seat_no, tf.fare_conditions, 
    t.passenger_name, t.contact_data -> 'phone' as phone 
...
select tf.flight_id, bp.boarding_no, bp.seat_no, tf.fare_conditions, 
    t.passenger_name, t.contact_data -> 'phone' as phone 
from tickets t join ticket_flights tf using (ticket_no) 
  join boarding_passes bp using (ticket_no, flight_id)
 where tf.flight_id = 42420;
```
