...
```pgsql
create sequence task_id_seq start with 1 increment by 1 maxvalue 99999;
```
...
returns trigger as $$
begin 
	select '500-'||to_char(nextval('task_id_seq'),'FM09999')||'/'||to_char(now(),'YYYY') into STRICT NEW.ID;
	return new;
end $$ language plpgsql;
```
